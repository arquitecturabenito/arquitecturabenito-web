...
for (let i = 0; i < lines.length; i++) {
   if (lines[i].includes("if (simulationRef.current) {") && lines[i+4] && lines[i+4].includes(".force('link'")) {
       anchorIndex = i + 3;
       break;
   }
}
...
if (anchorIndex !== -1) {
    const originalTextAfterAnchor = code.substring(code.indexOf(lines[anchorIndex]) + lines[anchorIndex].length);
    code = code.substring(0, code.indexOf(lines[anchorIndex]) + lines[anchorIndex].length) + missingCode + originalTextAfterAnchor.substring(originalTextAfterAnchor.indexOf(".force('link'"));
    
    fs.writeFileSync('index.html', code);
    console.log("Reconstructed code successfully!");
} else {
