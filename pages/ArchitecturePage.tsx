...
type Slide = (Project & { type: 'project' }) | { id: 'cover', type: 'cover', image: string, title: string };

export const ArchitecturePage: React.FC<ArchitecturePageProps> = ({ onBack, onProjectClick }) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const mainContainerRef = useRef<HTMLElement>(null);
  const slideRefs = useRef<(HTMLElement | null)[]>([]);

  const architectureProjects = allProjectsData.filter(p => p.category === 'architecture');
  // FIX: Explicitly type `coverSlide` to match the shape defined in the `Slide` union type.
  // This resolves the type inference issue where `id` was inferred as `string` instead of the literal `'cover'`.
  const coverSlide: { id: 'cover'; type: 'cover'; image: string; title: string } = {
    id: 'cover',
    type: 'cover',
    image: '/imagenes/telas-portada-1.png',
    title: 'Arquitectura'
  };
      ...
      <main ref={mainContainerRef} className="w-full h-full flex overflow-x-auto overflow-y-hidden snap-x snap-mandatory">
        {slides.map((slide, index) => (
          <section
            ...
            style={{ backgroundImage: `url('${slide.type === 'cover' ? slide.image : slide.normalImage}')` }}
          >
            ...
            <div className="relative z-10 text-center text-white p-4 transform group-hover:scale-105 transition-transform duration-500">
              ...
              {slide.type === 'project' && <p className="mt-4 text-sm uppercase tracking-widest border-b border-white pb-1 inline-block">Ver Proyecto</p>}
            </div>
